...
<head>
  ...
  -->
  <meta charset="utf-8" />
  <title>My Foursquare check-ins mapped</title>
  <style type="text/css">
    ...
    function getLocations() {
      $.getJSON('?json', function(data) {
        if(data) {
          ...
          <?php if($alt_title) echo 'data[0] = \''.$alt_title.'\';'; ?>document.title = data[0];$('#header h1').html(data[0] + ' <span><a href="' + data[1] + '">My Foursquare profile</a></span>');
          locations = data[2];
          for(var i = 0; i < data[2].length; i++) {setTimeout(function(){addMarker();},i*750);}
